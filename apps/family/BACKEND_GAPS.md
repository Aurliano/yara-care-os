...
14. **Two-Way Messaging & Voice Messages (Resolved)** — Two-way messaging with asynchronous voice, image, video, and text messages is fully implemented via `/api/v1/communication/elders/{id}/messages/` and `/api/v1/communication/media/upload/`. (Superseded the old ADR-014 stub).
