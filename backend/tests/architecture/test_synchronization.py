"""Architecture tests for Synchronization domain boundaries."""
...
def test_synchronization_has_no_cross_domain_fks():
    relations = collect_foreign_key_relations(app_labels={"synchronization"})
    cross_domain = [r for r in relations if r.from_app == "synchronization" and r.to_app != "synchronization"]
    assert cross_domain == []


def test_synchronization_cross_domain_fks_are_allowed():
    relations = collect_foreign_key_relations(app_labels={"synchronization"})
    cross_domain = [r for r in relations if r.from_app == "synchronization" and r.to_app != "synchronization"]
    allowed = {
...
def test_no_forbidden_synchronization_cross_domain_fks():
...
def test_synchronization_does_not_import_business_domains():
    forbidden = (
        "domains.workflow",
        "domains.care",
        "domains.device",
